...
// Main differences between original and this one
//   - Small adjustments to LCD_PrintChar(unsigned char data)
//   - commented out ADC_Init function (interferied with tiDefs.h
...
void LCD_GPIOInit(void){
    ...
    wait++;                                // wait for port activation
    GPIOBDIR |= 0xFF;                      // make PB0-7 outputs
    GPIOBAFSEL &= ~0xFF;                   // disable alternate functions 
    GPIOBDEN |= 0xFF;                      // enable digital I/O on PB0-7
    
    // activate control pins
    RCGCGPIO |= 0x1;                      // activate port A
    wait++;                               // wait for port activation
    wait++;                               // wait for port activation
    GPIOADIR |= 0xF0;                     // make PA4-7 outputs
    GPIOAAFSEL &= ~0xFF;                  // disable alternate functions 
    GPIOADEN |= 0xF0;                     // enable digital I/O on PA4-7
...
void LCD_PrintChar(unsigned char data){
    ...
    if (cursorX + 15 >= LCD_WIDTH) { // changed from 5
        LCD_SetCursor(10, cursorY + 9);
    }

    // If character would print beyond LCD_HEIGHT, return to top of screen
    if (cursorY + 18 >= LCD_HEIGHT) {// changed from 8
        LCD_SetCursor(cursorX, 10);
    }
...
void LCD_PrintString(char data[]){
    ...
    if ((cursorX + 25) + 15 >= LCD_WIDTH) {
      if((cursorY + 18 + 9 >= LCD_HEIGHT)) {
        LCD_ColorFill(convertColor(255, 255, 255)); // clear screen
      }
      LCD_SetCursor(10, cursorY + 9);
    }
